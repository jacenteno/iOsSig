...
struct InventoryAlertCardView: View {
  ...
  var body: some View {
    HStack(spacing: 16) {
      ...
      Image(
        systemName: alert.type == .outOfStock
      ...
      VStack(alignment: .leading, spacing: 4) {
        ...
        Text(
          "Cód: \(alert.product.codproducto ?? "N/A") | Ref: \(alert.product.codigobarra ?? "N/A")"
        )
        .font(.caption)
...
#Preview {
  let mockProductAgotado = Product(
    ...
    detalle: "", codigobarra: "12345", codproducto: "PROD-OUT", codproveedor: 1,
    referencia: "REF-OUT",
  ...
  let mockProductBajoStock = Product(
    ...
    detalle: "", codigobarra: "67890", codproducto: "PROD-LOW", codproveedor: 1,
    referencia: "REF-LOW",
  ...
  let alertAgotado = InventoryAlert(
    product: mockProductAgotado, currentStock: 0, lastMonthSales: 50, type: .outOfStock)
  let alertBajoStock = InventoryAlert(
    product: mockProductBajoStock, currentStock: 40, lastMonthSales: 100, type: .lowStock)
