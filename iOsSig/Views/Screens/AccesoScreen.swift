...
struct AccesoScreen: View {
        ...
        VStack(spacing: 12) {
            ZStack {
                Circle()
                    ...
                    .shadow(color: Color(hex: settings.accentColor)?.opacity(0.3) ?? .accentColor.opacity(0.3), radius: 10, x: 0, y: 5)
    ...
    private var passwordField: some View {
        ...
        .overlay(
            RoundedRectangle(cornerRadius: 16)
                .stroke(
                    passwordError ? Color.red.opacity(0.6) : Color(hex: settings.accentColor)?.opacity(0.3) ?? Color.accentColor.opacity(0.3),
                    lineWidth: 2
                )
        )
    ...
    private var unlockButton: some View {
        Button(action: attemptUnlock) {
            ...
            .background(
                LinearGradient(
                    colors: [Color(hex: settings.accentColor) ?? .accentColor, Color(hex: settings.accentColor)?.opacity(0.8) ?? .accentColor.opacity(0.8)],
                    startPoint: .topLeading,
                    endPoint: .bottomTrailing
                )
            )
            .cornerRadius(16)
            .shadow(color: (Color(hex: settings.accentColor) ?? .accentColor).opacity(0.4), radius: 8, x: 0, y: 4)
