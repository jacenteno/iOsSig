  ...
  var body: some View {
    VStack(alignment: .leading, spacing: 16) {
      ...
      VStack(alignment: .leading, spacing: 8) {
        InfoRow(
          label: "Fecha de Creación", value: formattedDate(from: order.createdAt), icon: "calendar")
        InfoRow(
          label: "Última Actualización", value: formattedDate(from: order.updatedAt),
...
struct ItemRow: View {
  ...
  private var progress: Double {
    ...
    if lowercasedStatus == "completado" || lowercasedStatus == "entregada"
      || lowercasedStatus == "despachada"
    {
      return 1.0
    }
    ...
    if lowercasedStatus == "procesando" {
      let calculatedProgress =
        item.quantityUnits > 0 ? item.quantityUnitsDispatched / item.quantityUnits : 0
  ...
  var body: some View {
    VStack(alignment: .leading, spacing: 8) {
      ...
      HStack {
        ...
        Spacer()
        Text(
          "Unidades Despachadas: \(item.quantityUnitsDispatched.formatted(withDecimalPlaces: 2))")
      }
      .font(.subheadline)
      .foregroundColor(.secondary)
    }
...
struct OrderDetailView_Previews: PreviewProvider {
  ...
  static var previewOrder: RequestOrderResponse {
    let user = UserResponse(
      employeeId: "1", username: "jdoe", email: "jdoe@example.com", fullName: "John Doe",
      createdAt: "2025-10-30T10:00:00Z")
    let product1 = ProductDetailResponse(
      indexProductos: 1, codProducto: "123", desProducto: "Test Product 1", ultCosto: 10.0,
      existencias: 100.0)
    let item1 = OrderItemResponse(
      product: product1, quantityUnits: 10, quantityBoxes: 1, quantityUnitsDispatched: 5,
      quantityBoxesDispatched: 0)
    let product2 = ProductDetailResponse(
      indexProductos: 2, codProducto: "456", desProducto: "Test Product 2", ultCosto: 20.0,
      existencias: 50.0)
    let item2 = OrderItemResponse(
      product: product2, quantityUnits: 20, quantityBoxes: 2, quantityUnitsDispatched: 20,
      quantityBoxesDispatched: 2)
