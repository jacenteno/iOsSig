...
struct ProductScreen: View, CameraScannerViewDelegate {
  ...
  @State private var isShowingReferenceSearch = false  // Estado para la búsqueda por referencia
    ...
    ZStack {
      ...
      .sheet(isPresented: $showError) {
        ErrorView(
          ...
          retryAction: { viewModel.fetchProducts() }, isShowingError: $showError)
      }
      ...
      .sheet(isPresented: $isShowingReferenceSearch) {
        ReferenceSearchView(isPresented: $isShowingReferenceSearch) { reference in
          viewModel.searchByReference(reference: reference)
        }
      ...
      .onChange(of: settings.selectedProductCodeForSearch) { newValue in
        print(
          "DEBUG: ProductScreen.onChange(selectedProductCodeForSearch) triggered with: \(newValue ?? "nil")"
        )
      ...
      if viewModel.showCreateProductAlert {
        CustomAlertView(
          ...
          message:
            "El producto con el código \(viewModel.productNotFoundCode ?? "") no existe. ¿Desea crearlo?",
  ...
  private var searchHeaderView: some View {
    VStack(spacing: 12) {
      ...
      HStack {
        ...
        TextField(
          "Buscar productos...", text: $viewModel.searchQuery,
          onCommit: {
            viewModel.searchProductByCode()
          }
        )
  ...
  private var contentView: some View {
    if viewModel.isLoading {
      ...
      EmptyStateView(
        systemImage: "magnifyingglass", message: "Busca productos por nombre, código o referencia.")
    } else {
      productsListView
    }
  }
...
struct ProductCardView: View {
  ...
  var body: some View {
    ...
    .background(
      VStack {
        NavigationLink(
          destination: HacerPedidosScreen().environmentObject(cartManager),
          isActive: $isHacerPedidosActive
        ) { EmptyView() }
        NavigationLink(destination: EtiquetaScreen(product: product), isActive: $isEtiquetaActive) {
          EmptyView()
        }
      }
    )
  ...
  private var detailsSection: some View {
    ...
    Button(action: {
      ...
      HStack {
        ...
        Image(
          systemName: cardViewModel.showDetails
    ...
    VStack(spacing: 12) {
      ...
      HStack(spacing: 12) {
        ...
        if settings.desplegarComprasApiOld
          && settings.hasPermission("Ver_Compras_en_Consulta_Producto")
        {
          ActionGridButton(
  ...
  var body: some View {
    VStack(spacing: 12) {
      ...
      if settings.hasPermission("VIEW_INVENTARIO") {
        let stockValue =
          cmdProductDetails?.existencia
          ?? (product.existencias != nil ? Int(product.existencias!) : 0)
        HStack(spacing: 12) {
