...
struct ClienteScreen: View {
  ...
  var body: some View {
    NavigationStack {
      ZStack {
        ...
        VStack(spacing: 0) {
          ...
          ModernSearchBar(text: $valor) {
            Task {
              await viewModel.fetchCliente(
                valor: valor.trimmingCharacters(in: .whitespacesAndNewlines))
            }
          }
          ...
          ScrollView {
            VStack(spacing: 24) {
              ...
              if viewModel.clienteResponse == nil && viewModel.error == nil
              ...
              if let promotions = viewModel.clienteResponse?.activePromotions, !promotions.isEmpty {
                PromocionesSectionView(
                  promotions: promotions, usedPromotions: viewModel.usedPromotions,
      ...
      .alert(
        "Error", isPresented: .constant(viewModel.error != nil),
        actions: {
      ...
      .alert(
        "¡Éxito!", isPresented: .constant(viewModel.successMessage != nil),
        actions: {
    ...
    if let cli = viewModel.clienteResponse?.cliente {
      VStack(alignment: .leading, spacing: 20) {
        ...
        ModernSectionView(title: "Información de Contacto", icon: "phone.circle.fill") {
          ModernLabeledContent(
            label: "Teléfono 1",
            value: cli.telefono1.trimmingCharacters(in: .whitespacesAndNewlines))
          ModernLabeledContent(
            label: "Teléfono 2",
            value: cli.telefono2.trimmingCharacters(in: .whitespacesAndNewlines))
          ModernLabeledContent(
            label: "Email", value: cli.email.trimmingCharacters(in: .whitespacesAndNewlines))
        }
        ...
        ModernSectionView(title: "Detalles de Cuenta", icon: "person.badge.key.fill") {
          ...
          ModernLabeledContent(
            label: "Cédula",
            value: cli.rut?.trimmingCharacters(in: .whitespacesAndNewlines) ?? "N/A")
        ...
        ModernSectionView(title: "Resumen del Programa", icon: "star.circle.fill") {
          ...
          ModernLabeledContent(
            label: "Total Facturas", value: String(format: "%.0f", cli.totalFacturas))
          ModernLabeledContent(
            label: "Consumo Total", value: String(format: "$%.2f", cli.consumido))
        }
  ...
  var body: some View {
    VStack(alignment: .leading, spacing: 14) {
      ...
      VStack(alignment: .leading, spacing: 6) {
        ...
        ModernPromoRow(
          icon: "calendar", label: "Válido", value: promotion.specificDate ?? "No especificada")
        ModernPromoRow(
          icon: "clock", label: "Horario",
          value: "\(promotion.specificStartTime ?? "N/A") - \(promotion.specificEndTime ?? "N/A")")
      }
      .font(.system(size: 14, weight: .medium))
      .foregroundColor(.secondary)
