...
struct SettingsView: View {
    ...
    private var apiSection: some View {
        Section {
            ModernTextField(icon: "shippingbox.circle.fill", title: "URL de Productos", text: Binding($tempSettings)!.productApiUrl, keyboard: .URL)
            ModernTextField(icon: "person.2.circle.fill", title: "URL de Clientes", text: Binding($tempSettings)!.clientApiUrl, keyboard: .URL)
            ModernTextField(icon: "d.circle.fill", title: "URL Citymall DAVID", text: Binding($tempSettings)!.citymallApiUrl, keyboard: .URL)
            ModernTextField(icon: "f.circle.fill", title: "URL Citymall FRONTERA", text: Binding($tempSettings)!.citymallFronteraApiUrl, keyboard: .URL)

            ModernToggle(icon: "arrow.left.arrow.right", title: "Usar API Old", isOn: Binding($tempSettings)!.useOldApi)
            ModernToggle(icon: "chart.line.uptrend.xyaxis", title: "Mostrar Ventas API OLD", isOn: Binding($tempSettings)!.desplegarVentasApiOld)
    ...
    private var printerSection: some View {
        Section {
            ...
            if tempSettings?.printerConnectionType == .wifi {
                ModernTextField(icon: "wifi", title: "IP Impresora", text: Binding($tempSettings)!.printerIpAddress, keyboard: .URL)
                ModernTextField(icon: "point.3.connected.trianglepath.dotted", title: "Puerto", text: Binding($tempSettings)!.printerPort, keyboard: .numberPad)
            }

            if tempSettings?.printerConnectionType == .bluetooth {
                ModernTextField(icon: "b.circle.fill", title: "MAC Address", text: Binding($tempSettings)!.printerMacAddress, capitalization: .allCharacters)
            }
        } header: {
