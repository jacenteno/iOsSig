...
struct CambioPrecioView: View {
  ...
  var body: some View {
          ...
          VStack(alignment: .leading, spacing: 16) {
            ...
            VStack(alignment: .leading, spacing: 8) {
              ...
              if let currentPrice = viewModel.product?.preciodeventa,
                let newPrice = Double(
                  viewModel.newPriceString.replacingOccurrences(of: ",", with: ".")),
                newPrice > 0
              {
                ...
                HStack(spacing: 8) {
                  Image(
                    systemName: difference >= 0 ? "arrow.up.circle.fill" : "arrow.down.circle.fill"
                  )
    ...
    .alert(isPresented: $viewModel.showAlert) {
      Alert(
        title: Text(
          viewModel.alertMessage.starts(with: "Precio actualizado") ? "✓ Éxito" : "⚠️ Atención"),
