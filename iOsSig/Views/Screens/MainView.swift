...
struct MainView: View {
  ...
  private var tabItems: [TabItem] {
    ...
    if settings.hasPermission("VIEW_DASHBOARD") {  // Add Dashboard with permission check
      items.append(
        TabItem(title: "Panel Vtas", icon: "chart.bar.fill", view: AnyView(DashboardScreen())))
    }
    // if role.hasPermission("VIEW_PRODUCTS") {
    if settings.hasPermission("VIEW_PRODUCTS") {
      items.append(
        TabItem(title: "Productos", icon: "barcode.viewfinder", view: AnyView(ProductScreen())))
    }
    ...
    if settings.hasPermission("VIEW_FRONTERA") {  // Add Frontera with permission check
      items.append(
        TabItem(title: "Frontera", icon: "shippingbox.fill", view: AnyView(FronteraScreen())))
    }
    // if role.hasPermission("VIEW_ORDERS_LIST") {
    if settings.hasPermission("VIEW_ORDERS_LIST") {
      items.append(
        TabItem(
          title: "Pedidos", icon: "list.bullet.rectangle.fill", view: AnyView(OrdersListScreen())))
    }
    // if role.hasPermission("VIEW_CLIENTS") {
    if settings.hasPermission("VIEW_CLIENTS") {
      items.append(
        TabItem(title: "CityPuntos", icon: "person.2.fill", view: AnyView(ClienteScreen())))
    }
    ...
    if settings.hasPermission("VIEW_PRODUCTS") {  // Assuming VIEW_PRODUCTS is sufficient for offline consultation
      items.append(
        TabItem(
          title: "Consulta Offline", icon: "magnifyingglass",
          view: AnyView(OfflineProductsView(isPresented: .constant(true)))))
    }
    return items
  }
  ...
  var body: some View {
    ...
    .sheet(
      ...
      onDismiss: {  // Add onDismiss action
