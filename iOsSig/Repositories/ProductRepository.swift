    ...
    func saveProducts(_ products: [Product]) async {
        ...
        await context.perform {
            for product in products {
                let entity = self.coreDataStack.fetchProduct(byCode: product.codproducto ?? "", in: context) ?? ProductEntity(context: context)
    ...
    func deleteProducts(byDepartment departmentName: String) async throws {
        ...
        await context.perform {
            ...
            do {
                ...
                self.logger.error("❌ Error deleting products from department \(departmentName): \(error.localizedDescription)")
    ...
    func fetchLocalProducts(filter: String) async -> [Product] {
        ...
        return await context.perform {
            ...
            if !filter.isEmpty {
                ...
                request.predicate = NSCompoundPredicate(orPredicateWithSubpredicates: [descriptionPredicate, codePredicate, barcodePredicate, refPredicate])
            }
