...
class APIServiceCMD {
  ...
  private let logger = Logger(
    subsystem: Bundle.main.bundleIdentifier ?? "com.app", category: "APIService")
  ...
  func consultaCodigoBarra(requestBody: ConsultaCodigoBarraRequest) async throws -> CitymallResponse
  {
    ...
    let postString =
      "json=\(jsonString.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed) ?? "")"
  ...
  func consultaCodigoBarraConBody(request: ConsultaCodigoBarraRequest) async throws
    -> CitymallResponse
  {
  ...
  func getProductDetails(productCode: String) async throws -> ProductDetails {
    ...
    guard !productCode.isEmpty else {
      ...
      return ProductDetails(
        costo: nil, costoFob: nil, existencia: nil, comprasRealizadas: nil, found: false)
    }
    ...
    do {
      ...
      if response.resultado.errorcode == 0 {
        ...
        return ProductDetails(
          ...
          existencia: response.resultado.existencias != nil
            ? Int(response.resultado.existencias!) : nil,
          comprasRealizadas: totalComprasRealizadas,
          found: true
        )
      } else {
        logger.info(
          "ℹ️ Producto no encontrado o error en la respuesta (errorcode: \(response.resultado.errorcode))"
        )
        return ProductDetails(
          costo: nil, costoFob: nil, existencia: nil, comprasRealizadas: nil, found: false)
      }
