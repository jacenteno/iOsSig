...
class APIService {
  ...
  func updateProductPrice(codigo: String, updateData: [String: Any]) async throws {
    ...
    guard
      let encodedCodigo = trimmedCodigo.addingPercentEncoding(
    ...
    guard (200...299).contains(httpResponse.statusCode) else {
      ...
      if let errorBody = String(data: data, encoding: .utf8) {
        ...
        let detailedError = APIError.serverError(
          statusCode: httpResponse.statusCode, detail: errorBody)
        throw detailedError
      } else {
  ...
  func getVentasMensuales(codproducto: String) async throws -> Venta {
    ...
    do {
      ...
      let trimmedCodProducto = ventaResponse.codproducto.trimmingCharacters(
        in: .whitespacesAndNewlines)
      return Venta(
        codproducto: trimmedCodProducto, detalle: ventaResponse.detalle,
        ventasMensuales: ventaResponse.ventas_mensuales, totalVendido: ventaResponse.total_vendido,
        totalMonto: ventaResponse.total_monto)
    } catch {
  ...
  func fetchAllRequests(statuses: [String]? = nil, createdAfter: Date? = nil) async throws
  ...
  func getOperatorById(operatorId: Int) async throws -> Operator {
    ...
    do {
      ...
      if let operatorDetail = allOperators.first(where: { $0.employeeId == String(operatorId) }) {
        ...
        print(
          "APIService: Operator with employee ID \(operatorId) not found in the list of operators.")
  ...
  private func performRequest<T: Decodable>(request: URLRequest, validStatusCodes: [Int])
    async throws -> T
  {
    do {
      let (data, response) = try await session.data(for: request)
  ...
  func getAgotadoProducts(page: Int, searchQuery: String? = nil) async throws
    -> PaginatedAgotadoProductResponse
  {
  ...
  func fetchUserRequests(employeeId: String, statuses: [String]) async throws
