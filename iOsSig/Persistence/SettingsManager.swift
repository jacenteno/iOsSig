...
class SettingsManager: ObservableObject {
  ...
  private init() {
    ...
    self.citymallApiUrl =
      defaults.string(forKey: Keys.citymallApiUrl) ?? "http://138.118.127.162:8088/"
    self.citymallFronteraApiUrl =
      defaults.string(forKey: Keys.citymallFronteraApiUrl) ?? "http://138.118.127.162:8088/"
    ...
    self.userRole =
      AppUserRole(rawValue: defaults.string(forKey: Keys.userRole) ?? AppUserRole.ROL_0.rawValue)
      ?? .ROL_0
    self.isActivated = defaults.bool(forKey: Keys.isActivated)

    self.printerConnectionType =
      PrinterConnectionType(rawValue: defaults.string(forKey: Keys.printerConnectionType) ?? "None")
      ?? .none
    ...
    self.appColorScheme =
      AppColorScheme(
        rawValue: defaults.string(forKey: Keys.appColorScheme) ?? AppColorScheme.system.rawValue)
      ?? .system
    self.accentColor =
      defaults.string(forKey: Keys.accentColor) ?? Color.customPrimary.toHex() ?? "#FF0000"  // Default to red if conversion fails
  ...
  private func loadOrSeedRolePermissions() {
      ...
      let decodedPermissions = try? JSONDecoder().decode([String: Set<String>].self, from: data)
    {
      ...
      print(
        "SettingsManager: No se encontraron permisos guardados. Se inicializaron desde el modelo estático."
      )
    }
  }
