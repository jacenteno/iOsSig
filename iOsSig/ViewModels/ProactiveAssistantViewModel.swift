...
class ProactiveAssistantViewModel: ObservableObject {
    ...
    func generateAlerts(isNewSearch: Bool = false) async {
        ...
        do {
            let paginatedResponse = try await apiService.getAgotadoProducts(page: 1, searchQuery: searchQuery)
            let agotadoProducts = paginatedResponse.results
            
            var newAlerts: [InventoryAlert] = []
            for product in agotadoProducts {
                let alert = InventoryAlert(product: product.toProduct(), currentStock: 0, lastMonthSales: product.ventas_mes_anterior, type: .outOfStock)
