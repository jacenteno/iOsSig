...
class SyncProductsViewModel: ObservableObject {
    ...
    private init(apiService: APIService = APIService.shared, productRepository: ProductRepository = ProductRepository.shared) {
            ...
            do {
                ...
                try await withThrowingTaskGroup(of: Int.self) {
                    ...
                    for deptId in selectedIDsArray {
                        group.addTask {
                            let response = try await self.apiService.getProductsByDepartment(coddep: String(deptId), page: 1)
                ...
                for deptId in selectedIDsArray {
                    try Task.checkCancellation()
                    guard let deptName = departments.first(where: { $0.coddepartamento == deptId })?.nomdepto else { continue }
                    ...
                    while hasNextPage {
                        try Task.checkCancellation()
                        let response = try await apiService.getProductsByDepartment(coddep: String(deptId), page: currentPage)
                ...
                await MainActor.run {
                    syncMessage = "¡Sincronización completada! Se procesaron \(self.newItemsSynced) productos."
                    isSyncing = false
                }
                await updateLocalProductsCount()
