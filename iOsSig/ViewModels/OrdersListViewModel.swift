...
class OrdersListViewModel: ObservableObject {
  ...
  @Published var endDate: Date = {
    ...
    return Calendar.current.date(
      byAdding: .day, value: 1, to: Calendar.current.startOfDay(for: today))!.addingTimeInterval(-1)
  }()
  {
    didSet {
      applyFilters()
  ...
  private func applyFilters() {
    ...
    if selectedStatusFilter != .all {
      tempFiltered = tempFiltered.filter {
        $0.status.lowercased() == selectedStatusFilter.rawValue.lowercased()
      }
    ...
    tempFiltered = tempFiltered.filter { order in
      ...
      print(
        "Order ID: \(order.id), Order Date: \(orderDate), Start of Day: \(startOfDay), End of Day: \(endOfDay), Is Within Range: \(isWithinRange)"
      )
