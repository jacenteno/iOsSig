...
class CambioPrecioViewModel: ObservableObject {
    ...
    func updatePrice(codigo: String) async {
        ...
        if let priceList = product.listas_de_precio?.first {
            let isTemporaryActive = isTemporaryPriceActive(fechapreciotemp1: priceList.fechatemp1, fechapreciotemp2: priceList.fechatemp2)
            print("Revisando precio temporal. Activo: \(isTemporaryActive)")
            if isTemporaryActive {
                alertMessage = "Este producto tiene un precio temporal activo y no se permite cambiar el precio."
                showAlert = true
    ...
    private func convertClarionDate(clarionDate: Int) -> Date {
        var epoch = Calendar.current.date(from: DateComponents(year: 1800, month: 12, day: 28, hour: 0, minute: 0, second: 0))!
