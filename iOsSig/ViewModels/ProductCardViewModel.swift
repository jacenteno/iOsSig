...
class ProductCardViewModel: ObservableObject {
  ...
  private let logger = Logger(
    subsystem: Bundle.main.bundleIdentifier ?? "com.app", category: "ProductCardViewModel")  // New: Logger
  ...
  init(
    ...
    apiServiceCMD: APIServiceCMD = APIServiceCMD(session: .shared)
  ) {  // Modified init
  ...
  func loadProductDetails(for product: Product) {
    guard let productCode = product.codproducto?.trimmingCharacters(in: .whitespacesAndNewlines),
      !productCode.isEmpty
    else {
    ...
    Task {
      ...
      logger.info(
        "🌐 Intentando obtener detalles del producto de APIServiceCMD para: \(productCode)")
      do {
        ...
        if cmdDetails.found {
          ...
          logger.info(
            "ℹ️ APIServiceCMD no encontró el producto o los detalles. Intentando con APIService (fallback)."
          )
          ...
          self.cmdProductDetails = ProductDetails(
            ...
            existencia: fallbackProduct.existencias != nil
              ? Int(fallbackProduct.existencias!) : nil,
        ...
        logger.error(
          "❌ Error al cargar detalles del producto (CMD o fallback): \(error.localizedDescription)")
  ...
  func fetchVentas(for product: Product, completion: @escaping (Bool) -> Void) {
    guard let codproducto = product.codproducto?.trimmingCharacters(in: .whitespacesAndNewlines)
    else {
      completion(false)
      return
    }
  ...
  func fetchCompras(for product: Product, completion: @escaping (Bool) -> Void) {
    ...
    Task {
      ...
      logger.info(
        "🌐 Llamando a APIServiceCMD (a través de APIService) para obtener compras para: \(productCode)"
      )
      do {
