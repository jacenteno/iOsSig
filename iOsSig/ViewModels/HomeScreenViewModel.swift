...
class HomeScreenViewModel: ObservableObject {
    ...
    private func fetchOrders() async {
        ...
        do {
            let fetchedOrders = try await apiService.fetchAllRequests(statuses: statusesToFetch, createdAfter: date30DaysAgo)
            ...
            let summary = groupedByUsername.map { (username, orders) -> OperatorOrderSummary in
                ...
                return OperatorOrderSummary(username: username, pendingCount: pending, processingCount: processing)
            }.sorted { $0.username < $1.username } // Sort alphabetically by username

            DispatchQueue.main.async {
                self.orders = fetchedOrders
                self.operatorSummary = summary
                print("HomeScreenViewModel: Fetched and processed \(self.orders.count) active request orders.")
            }
        } catch {
            DispatchQueue.main.async {
    ...
    private func fetchPendingReceipts() async {
        ...
        do {
            let paginatedResponse = try await apiService.fetchReceipts(status: "pendiente", createdAfter: startOfMonth)
            DispatchQueue.main.async {
                self.receptionPendingCount = paginatedResponse.count
            }
        } catch {
            DispatchQueue.main.async {
                print("HomeScreenViewModel: Failed to fetch pending receipts: \(error.localizedDescription)")
            }
        }
    }
